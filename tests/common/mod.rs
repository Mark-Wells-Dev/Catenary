...
pub struct BridgeProcess {
    ...
    /// Stashed response from IPC-redirected `tools/call` in [`send`].
    ///
    /// When [`send`] intercepts a `tools/call` for grep/glob, it executes
    /// the query via IPC and stashes the synthetic MCP response here.
    /// [`recv`] returns it on the next call instead of reading from stdout.
    pending_ipc_response: Option<Value>,
}
...
impl BridgeProcess {
    ...
    pub fn send(&mut self, request: &Value) -> Result<()> {
        // Intercept tools/call for grep/glob and redirect via IPC.
        if request.get("method").and_then(|m| m.as_str()) == Some("tools/call")
            && let Some(name) = request.pointer("/params/name").and_then(|n| n.as_str())
            && (name == "grep" || name == "glob")
        {
            let args = request
                ...
                .cloned()
                .unwrap_or_else(|| json!({}));
            let id = request.get("id").cloned().unwrap_or_else(|| json!(100));
            let text_result = if name == "grep" {
                self.call_grep(&args)
            } else {
                self.call_glob(&args)
            };
            let response = match text_result {
                Ok(text) => json!({
                    "jsonrpc": "2.0",
                    "id": id,
                    "result": {
            ...
            self.pending_ipc_response = Some(response);
            return Ok(());
        }
    ...
    pub fn recv(&mut self) -> Result<Value> {
        // Return stashed IPC response if one was generated by send().
        if let Some(response) = self.pending_ipc_response.take() {
            return Ok(response);
        }
