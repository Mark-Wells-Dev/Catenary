...
- **Testing:**
  ...
  - Integration test subprocesses (bridge, `catenary install`, etc.) must call `isolate_env(&mut cmd, root)` **before** setting any `CATENARY_*` env vars. `isolate_env` clears all inherited `CATENARY_*` vars, then points each XDG base dir at a **distinct** subdir of the tempdir root: `XDG_CONFIG_HOME` → `<root>/config`, `XDG_STATE_HOME` → `<root>/state`, `XDG_DATA_HOME` → `<root>/data`, `XDG_RUNTIME_DIR` → `<root>/runtime` — and sets the `CATENARY_STATE_DIR`/`CATENARY_RUNTIME_DIR`/`CATENARY_CACHE_DIR`/`CATENARY_CONFIG_DIR`/`CATENARY_DATA_DIR` overrides to the same subdirs (the portable leg: the `dirs` crate ignores XDG vars on macOS, where the overrides carry the whole isolation), plus `CATENARY_HOME_DIR` → `<root>/home` (a sixth distinct base, bug 149) for home-rooted host artifacts (`~/.claude`, `~/.gemini`) resolved via `crate::paths::home_dir()` — `$HOME` itself is deliberately never redirected; test-side derivation is `common::catenary_home(root)`. A `clippy.toml` `disallowed-methods` gate denies direct `dirs::*` base-dir calls outside `src/paths.rs`, so every new base-rooted path needs a resolver with its own override there; the other route into the same hazard — reading `HOME` straight out of the environment, which clippy cannot deny because it cannot key on an argument — is closed by a source-scan pin test in `src/paths.rs` (misc 229): production code resolves home only through `paths::home_dir()`, `#[cfg(test)]` code is exempt, and `paths::compress_home()` is the one `~`-compressing display helper. Keeping the bases distinct makes `isolate_env` a mislocation detector — code that writes under the wrong base no longer silently lands in one shared dir. Callers then set `CATENARY_SERVERS`, `CATENARY_ROOTS`, or `CATENARY_CONFIG` after the call — these overwrite the cleared values. Test-side code that resolves a daemon path (socket, snapshot, or firehose log under `paths::state_dir()`) or writes a config the subprocess reads must derive it through `common::xdg_state_home(root)` / `common::xdg_config_home(root)` so both sides agree. Without `isolate_env`, subprocesses inherit the user's shell environment, writing to `~/.config`, `~/.local/state`, or `~/.local/share` and causing races between parallel tests and across worktrees.
