...
fn resolve_segment(cmd: &SimpleCommand, state: &mut State, ctx: SegCtx) -> SegmentClass {
    ...
    let class = match name {
        ...
        // Every `catenary` form lands here too (regime-1 canonical matcher):
        // the one write verb, `worktree land`, retired to a teaching stub
        // (wf-03) — the git-native merge flow carries landing (and its debt
        // transfer) now.
