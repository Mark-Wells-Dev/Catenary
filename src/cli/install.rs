...
mod tests {
    ...
    fn install_writes_pointer_per_host() {
        ...
        // guidance (Decision 12 / ticket 10). Claude is now the sole pointer host:
        // it carries a thin pointer to the primer, not a copy — so a new or changed
        // host is a one-line pointer, never a re-authoring. This is the file the
        // marketplace serves.
        //
        // Gemini and Antigravity are the exceptions (teaching-surface 06 / 10):
        // their shipped static artifacts demote to bootstrap/fallbacks that inline
        // the SSOT teaching (runtime data excluded) rather than pointing at the
        // primer. The live surface rides the runtime channel — Gemini's SessionStart
        // `additionalContext`, Antigravity's `PreInvocation` `userMessage` — while
        // the static artifact is the compaction-proof leg. Their freshness is pinned
        // in `cli::teaching` (`shipped_gemini_context_is_fresh`,
        // `shipped_antigravity_rules_are_fresh`) instead. OpenCode ships no static
        ...
        assert!(
            CLAUDE_SKILL.contains("catenary primer"),
            "claude instruction surface should point at `catenary primer`",
        );
