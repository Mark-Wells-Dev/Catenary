...
/// plugin's `config` hook; Gemini retired its context file in teaching-surface 14
/// — its teaching is hook-only, `SessionStart` plus the `PreCompress`/`BeforeAgent`
/// discontinuity re-injection.)
#[must_use]
...
// ── Runtime rules-file delivery (teaching-surface ticket 12; Gemini leg retired
//    in ticket 14 — this is now the Antigravity rules file only) ───────────────
